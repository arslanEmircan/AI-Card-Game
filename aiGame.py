...
class Card:
    ...
    def __hash__(self):
        return hash((self.suit, self.value)) #Kartın kontrolünü yapar, değer ve suit eşitse bunlar aynı kartlardır ve bu kartların hash değerleri aynıdır. aynı kartı tekrar kullanmamak için.
...
#Her suitten kaç tane var? En iyi suit hangisi? O suitten kaç kart var
...
def get_winning_probability(hand: List[Card], remaining_cards: int, difficulty: str) -> float:
    ...
    if cards_needed <= 0:
        return 1.0
    return max(0.0, min(1.0, (remaining_cards / 13) / cards_needed)) #Normalizasyon adımı, çıkan değer 1 den büyükse 1, 0dan küçükse 0 a eşitle
...
#Zor modun algoritması
    ...
    for suit in suits_in_hand:
        ...
        elde_weight = min(1.0, max(0.0, elde_weight))  # normalizasyon 1 den büyükse sonuç 1 e eşitle, 0 dan küçükse 0 a eşitle
        ...
        for card in comp_hand:
            if card != comp_new_card:
                ...
                if img:
                    ...
                    lbl.image = img  #  Referansı korumazsan görsel görünmez
